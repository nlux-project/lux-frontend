...
export const getUsername = (auth: AuthContextProps):
    string|undefined => {
      if (auth.user) {
...
export const getFormattedUuidForSubmission = (uuid: string):
    string => {
...
export const getBaseCollectionObject = ():
    IEntity => {
      return {
        type: 'Set',
      }
    ...
    Array<IConcept> => {
      if (!isUndefined(listOfUuids)) {
        return listOfUuids.map((uuid) => {
          return {
            id: uuid === config.aat.personalCollection ?
 ...
 * @param {Array<string>} listOfRecordIds the list of record UUIDs to add to the
 *     collection
 ...
 * @param {Array<string>} classifications the user entered classifications of
 *     the name of the collection being created
 * @param {Array<string>} languages the user entered languages of the name of
...
export const createCollectionObject = (
    ...
    IMyCollection => {
      // create new object for adding the name of the collection and its
      ...
      const nameClassificationsToAdd =
          getFormattedUuids('Type', classifications)
 ...
 * @param {Array<string>} listOfRecordIds the list of record UUIDs to delete
 *     from the collection
 * @returns {IMyCollection}
 */
 ...
 * Returns the collection JSON-LD object to be passed to the backend
 * @param {IMyCollectionObject} collection the collection JSON-LD to add to
 * @param {Array<string>} listOfClassifications the list of classifications to
...
export const addClassificationsToCollectionObject = (
    ...
    IMyCollection => {
      ...
      const classificationsToAdd =
          getFormattedUuids('Type', listOfClassifications)
 ...
 * Returns the collection JSON-LD object to be passed to the backend
 * @param {IMyCollectionObject} collection the collection JSON-LD to add to
 * @param {Array<string>} listOfIdentifiers the list of identifiers to add to
 *     the collection
 * @returns {IMyCollection}
 */
 ...
 * Returns the collection JSON-LD object to be passed to the backend
 * @param {IMyCollectionObject} collection the collection JSON-LD to add to
 * @param {Array<IWebpages>} listOfWebpages the list of identifiers to add to
 *     the collection
 * @returns {IMyCollection}
 */
export const addWebpagesToCollectionObject = (
  ...
  const languagesToAdd = getFormattedUuids('Language', languages)
    ...
    return {
    id: '', type: 'LinguisticObject', _label: 'Website Text',
 ...
 * Returns the collection JSON-LD object to be passed to the backend
 * @param {IMyCollectionObject} collection the collection JSON-LD to add to
 * @param {Array<INoteContent>} listOfIdentifiers the list of identifiers to add
 *     to the collection
 * @returns {IMyCollection}
 */
export const addNotesToCollectionObject = (
    ...
    IMyCollection => {
      ...
      const notesToAdd = listOfNotes.map((note) => {
        const {content, label, labelLanguages, classifications, languages} =
            note
        ...
        const labelLanguagesToAdd =
            getFormattedUuids('Language', labelLanguages)

        // Format label data
        const identifiedByToAdd: IEntity = !isUndefined(label) && label !== '' ?
            {type: 'Name', content: label} :
            {type: 'Name'}
            // Add label languages to the label object only if the
            // identifiedByToAdd object is not empty
            if (Object.prototype.hasOwnProperty.call(
                    identifiedByToAdd ?? {}, 'content')) {
...
export const formatSubTabNavLinks = (
  ...
  } if (!Object.prototype.hasOwnProperty.call(parsed ?? {}, 'AND')) {
    parsed.AND = []
  }
