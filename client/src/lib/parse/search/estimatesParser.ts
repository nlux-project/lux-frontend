...
export const transformAdvancedSearchEstimates = (
    ...
    Record<string, number|string> => {
      const estimates: Record<string, number|string> = {} if (data) {
        if (Object.prototype.hasOwnProperty.call(data ?? {}, tab)) {
        ...
        if (!isUndefined(subTab) &&
            Object.prototype.hasOwnProperty.call(data ?? {}, subTab)) {
...
export const transformSimpleSearchEstimates = (
    data:|Array<Record<string, IEstimateItems|ISearchResultsErrorData>>|
    undefined,
    ):
    Record<string, number|string> => {
      const estimates:
          Record<string, number|string> = {} if (data && data.length > 0) {
        for (const d of data) {
...
export const getSimpleSearchEstimates = (
    ...
    Promise<Record<string, any>[]|{data: Record<string, string|number>}> => {
      const promises = Object.keys(params).map((key: string) => {
        ...
        if (!Object.keys(nestedPageLinks).includes(key)) {
          ...
          subTab = key
          const updatedParams =
              formatSubTabNavLinks(user, subTab, params[subTab])
          urlParams.set('q', updatedParams)
        }
        return fetchSearchEstimates(
            urlParams.toString(), searchScopeTab, subTab)
      })
 ...
 * @param {boolean} isSuccess; value determining if the estimates requests was
 *     successful
