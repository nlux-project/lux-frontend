...
import {collectionsIcon, conceptsIcon, eventsIcon, objectsIcon, peopleOrgsIcon, placesIcon, softwareElectronicMediaIcon, textualWorksIcon, visualWorksIcon,} from '../../../config/resources'
...
import {forceArray, getClassifiedAs, getEquivalentFromClassifiedAsArray, getIdentifiedByContent, getMultipleSpecificReferredToBy, getName, getNestedCarriedOutBy, getSpecificReferredToBy, isEquivalent, sortDataSources, validateClassifiedAsIdMatches,} from './helper'
...
export default class EntityParser {
  ...
  /**
   * Returns an array of strings containing related web pages from
   * /subject_of/digitally_carried_by/access_point
   * @returns {string}
   */
  getEntityClass(scope: string): string{const {type} = this.json
  ...
  getWebPages(): Array<string> {
      const urls: Array<string> = [] const subjectOf =
          forceArray(this.json.subject_of)
    ...
    /**
     * Returns the IIIF manfiest from
     * /subject_of/digitally_carried_by/conforms_to/
     * @returns {string}
     */
     ...
     * @returns {string}
     */
    getPrimaryName(lang: string):
        string{const identifiedBy = forceArray(this.json.identified_by)
  ...
  getNames(removePrimaryName?: boolean): IContentWithLanguage | null {
      ...
      identifiedBy.filter((identifier) => identifier.type === 'Name')
          .map((identifier) => {
            ...
            // The label will be an empty string if it does not come from the
            // data
            if (classifiedAs.length === 0 && identifiedByChild.length === 0) {
              label = ''
            }

            // Get the label of the list of names from either the nested
            // classified_as or nested identified_by
            if (classifiedAs.length > 0) {
              // Filter out inverted terms classifications
              const ids =
                  getClassifiedAs(classifiedAs, [config.aat.invertedTerms])
            ...
            if (identifier.content !== undefined) {
              if (Object.prototype.hasOwnProperty.call(data ?? {}, label)) {
                ...
                data[label] =
                    [{content: identifier.content, language, notation}]
              }
            }
            return null
          })
     ...
     */
    getRepresents():
        Array<string>{const represents = forceArray(this.json.represents)
  ...
  getDataSources(): Record<string, Array<string>> {
      ...
      return {
        ...recordId, ...formattedManifest, ...yaleRecordsSorted,
    ...
    getCopyrightLicensingStatement():
        Array<INoteContent>{return getSpecificReferredToBy(
            this.json,
     ...
     */
    getPlanYourVisitLink():
        Array<INoteContent>{return getMultipleSpecificReferredToBy(
            this.json, config.aat.visitors)}
      ...
      const imageReps: IImages[] = []
        ...
        for (const digital of digitallyShownBy) {
          ...
          const copyrightStatements = new EntityParser(
                                          ...
                                          .getCopyrightLicensingStatement()
          imageRep.attribution = copyrightStatements.length > 0 ?
              copyrightStatements[0].content :
              ''
    ...
    getNotes(): IContentWithLanguage|null {
      ...
      referredToBy.map((el: IEntity) => {
        ...
        // do not parse the entity and return null as they should not be
        // displayed with notes
        if (!isUndefined(el.classified_as)) {
          if (validateClassifiedAsIdMatches(el.classified_as, [
                config.aat.copyrightLicensingStatement,
                config.aat.visitors,
        ...
        if (label === undefined) {
          ...
          label = labelClassifications.length > 0 ? labelClassifications[0] :
                                                    'Additional Notes'
        }
        ...
        if (contentToDisplay !== undefined || htmlContent !== undefined) {
          if (Object.prototype.hasOwnProperty.call(data ?? {}, label)) {
            // If the content has a language and it is english we want to push
            // it to the front of the array so it gets displayed first
            if (isEnglish) {
              data[label].unshift({
    ...
    /**
     * Returns the supertype icon to be displayed with the entity along with its
     * alt text
     * @returns {Array<string>}
     */
    getSupertypeIcon(): Array<string> {
      ...
      switch (type) {
        ...
        return [objectsIcon, 'Physical Objects'] case 'DigitalObject':
        return [
          softwareElectronicMediaIcon, 'Digital Objects'
        ] case 'VisualItem':
        return [visualWorksIcon, 'Visual Works'] case 'Set':
        return [collectionsIcon, 'Collections'] case 'LinguisticObject':
        return [textualWorksIcon, 'Textual Works'] case 'Person':
        return [peopleOrgsIcon, 'People & Groups'] case 'Group':
        return [peopleOrgsIcon, 'People & Groups'] case 'Place':
        return [placesIcon, 'Places'] case 'Currency':
        return [conceptsIcon, 'Concepts'] case 'Language':
        return [conceptsIcon, 'Concepts'] case 'Material':
        return [conceptsIcon, 'Concepts'] case 'MeasurementUnit':
        return [conceptsIcon, 'Concepts'] case 'Type':
        return [conceptsIcon, 'Concepts'] case 'Activity':
        return [eventsIcon, 'Events'] case 'Period':
        return [eventsIcon, 'Events'] case 'Event':
          return [eventsIcon, 'Events'] default: return ['', '']
      }
    }
      ...
      const links = []
        ...
        for (const digital of digitallyCarriedBy) {
          ...
          if (classifiedAs !== undefined) {
            if (validateClassifiedAsIdMatches(
                    classifiedAs, [config.aat.webPage])) {
              for (const p of accessPoint) {
    ...
    /**
     * Returns array of transformed link data from /subject_of that are not web
     * pages or IIIF manifests
     * @returns {Array<IWebpages>}
     */
     ...
     * @returns {Array<{label: string; identifier: Array<string>; carriedOutBy:
     *     Array<string>}>}
     */
    getIdentifiers(): Array < {label: string
    ...
    const agent = [
      ...
      ...getNestedCarriedOutBy(assignedBy),
        ...
        return {
      label, identifier: identifier.content, carriedOutBy: agent,
          equivalent: equivalentObjects,
        }
      })

        const identifierData: Array < {label: string
      identifier: Array<string>
      carriedOutBy: Array<string>
      equivalent: Array<string>
    }> = []
    ...
    for (const identifier of identifiers) {
        ...
        if (existing.length > 0) {
          ...
          identifierData[existingIndex].identifier =
              identifierData[existingIndex].identifier.concat(
     ...
     */
    getAboutSubsection():
        Array<Array<string>|string>{const about = forceArray(this.json.about)
    const aboutIds = getClassifiedAs(about)

    const aboutData =
        about
            .map((obj) => {
              const ids: Array<string> = [] if (
                  Object.prototype.hasOwnProperty.call(
                      obj ?? {}, 'created_by')) {
                obj.created_by.influenced_by.map(
                    (influencedBy: IEntity) => {ids.push(influencedBy.id!)})
              }
              return ids
            })
