...
import {addRemoveConfig, getDefaultSearchOptions, QueryOption,} from '../../config/advancedSearch/options'
import config from '../../config/config'
import {scopeToAriaLabel} from '../../config/searchTypes'
import {IAdvancedSearchState} from '../../redux/slices/advancedSearchSlice'

import {getProperty, isBooleanInput,
        // isDateInput,
        isGroup, isInput, isRangeInput, isRecordTypeInput,} from './advancedSearchParser'
...
export const getSingleFieldDropdownOptions = (
    ...
    Record<string, Record<string, string>> => {
      ...
      const parentLabels: Record<string, Record<string, string>> =
          {} Object.entries(config.advancedSearch.terms[parentScope])
              .map(
                  ([key, value]) => {
                    const valueObj = value as Record<string, string>
                        // If the search term is not specific to one scope
                        if (!Object.keys(scopeToAriaLabel)
                                 .includes(valueObj.relation)) {
...
export const getFieldToEntityRelationship = (
    ...
    string|null => {
      const searchTermConfig =
          config.advancedSearch
                  .terms[parentScope][searchTerm] return searchTermConfig !==
              undefined ?
          searchTermConfig.relation :
          null
    }
...
export const getExistingValue = (
    ...
    IAdvancedSearchState|null => {
      ...
      if (typeof existingValue === 'string' &&
          Object.prototype.hasOwnProperty.call(comparators, existingValue)) {
        existingValue = null
      }
      delete obj[property]
    ...
    Record<string, any>|null => {
      if (objectToUpdate !== null) {
        ...
        if (Object.keys(conditionals).includes(selected)) {
          ...
          // Text input
          if (!isRecordTypeInput(selected) &&
              (typeof existingValue !== 'string' || isRangeInput(selected) ||
               isBooleanInput(selected))) {
            objectToUpdate[selected] = '' delete objectToUpdate._comp
          } else {
            objectToUpdate[selected] =
...
/**
 * Retreives updated search options based on the current options selected for a
 * field
 * @param {QueryOption} selectedOption current option selected in the advanced
 *     search state
 * @param {QueryOption} currentOptions options available for the given search
 *     term
 * @returns {QueryOption[]}
 */
export const getUpdatedOptions = (
    ...
    QueryOption[] => {
      const {optionsToAdd, optionsToRemove} =
          addRemoveConfig[selectedOption] const updatedOptions =
 ...
 * @param parentObject IAdvancedSearchState; Parent object from which to remove
 *     child
 ...
 * @param {string} scope entity scope of the current search, such as item, work,
 *     places, etc
...
export const convertAqSearchParam = (
    ...
    IAdvancedSearchState => {
      ...
      // Remove scope as it is not needed for rendering and scope gets passed
      // via the current tab
      ...
      if (!Object.prototype.hasOwnProperty.call(jsonAqParamValue, '_stateId')) {
      ...
      for (const key of keys) {
        if (!Object.prototype.hasOwnProperty.call(conditionals, key) &&
          ...
          // On submit, we will convert back to 'exact' if all component options
          // are selected
        ...
        if (key !== '_options') {
          ...
          if (Array.isArray(nestedObject)) {
            jsonAqParamValue[key] = nestedObject.map(
                (obj) => convertAqSearchParam(scope, obj, newBgColor),
            )
          }
        }
      }
