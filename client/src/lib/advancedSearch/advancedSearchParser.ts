...
import {convertLuxISODateToISODate, isValidDateObject,} from '../facets/dateParser'
 ...
 * @returns string
 */
export const getProperty = (obj: IAdvancedSearchState):
    string => {
...
export const isInput = (searchTerm: string): boolean =>
    isRangeInput(searchTerm) || isTextInput(searchTerm) ||
    isBooleanInput(searchTerm) || isRecordTypeInput(searchTerm) ||
 ...
 * @returns boolean
 */
export const isChildAGroup = (obj: IAdvancedSearchState):
    boolean => {
 ...
 * @returns boolean
 */
export const isChildAnInputField = (obj: IAdvancedSearchState):
    boolean => {
      if (isEmptyObj(Object.keys(obj))) {
        return false
      }
...
export const isSingleRelationshipField = (
    ...
    boolean => {
      const property = getProperty(obj)
      return (
          !isInput(property) && !isAGroup(property) &&
          !isEmptyObj(Object.keys(obj)))
    }

/**
 * Determines if the properties given contains a property that requires user
 * input
 * @param properties Array<string>; array of properties
 * @returns boolean
 */
export const containsInput = (properties: Array<string>):
    boolean => {
...
export const validateAdvancedSearch = (
    ...
    boolean => {
      ...
      const nested = state[property]
          ...
          // If the nested property is text input and it is a string value that
          // is not empty
          if (isTextInput(property) || isRecordTypeInput(property)) {
        const nestedClone = nested as string
        // Text input should not allow for strings containing only double quotes
        // and/or spaces
      ...
      // If the nested property is range input and it is a string value and
      // comparator that is not empty
      if ((isRangeInput(property) || isDateInput(property)) && nested !== '' &&
          Object.prototype.hasOwnProperty.call(state, '_comp') &&
          state._comp !== '') {
        return true
      }
      ...
      if (Array.isArray(nested)) {
        const vals: Array<boolean> = [] for (const obj of nested) {
 ...
 * @returns boolean
 */
export const filterAdvancedSearch = (scope: string, state: any):
    any => {
      const currentState = _.cloneDeep(state)

      // remove _stateId as it is only used on the frontend and is not valid in
      // the backend
      if (Object.prototype.hasOwnProperty.call(currentState, '_stateId')) {
        delete currentState._stateId
      }

      // remove _bgColor as it is only used on the frontend and is not valid in
      // the backend
      if (Object.prototype.hasOwnProperty.call(currentState, '_bgColor')) {
      ...
      if (isDateInput(propertyToCheck)) {
        const value = currentState[propertyToCheck] const dateObj =
            new Date(value)
      ...
      // Is range
      if (Object.prototype.hasOwnProperty.call(currentState, '_comp')) {
        if (currentState._comp === '') {
          return null
        }
      }

      // we want to shrink the _options param as small as possible, so that we
      // don't make our request unnecessarily large
      if (Object.prototype.hasOwnProperty.call(currentState, '_options')) {
        ...
        // if there are options selected that ML applies by default, let's not
        // them in the request
      ...
      if (!Array.isArray(currentState[propertyToCheck]) &&
        ...
        // if the property is numeric (inside an array), simply pass down the
        // scope
        if (/[0-9]/.test(propertyToCheck)) {
          ...
          // }
          // since the property is none of the following: null, array, numeric
          // string, it must be a valid search term. Recurse with the new target
          // scope for this search term else if
          // (!Object.keys(text).includes(propertyToCheck)) {
...
export const getAdvancedSearchDepth = (state: IAdvancedSearchState): number => {
  ...
  const nested = state[property]
      ...
      // If the nested property is text input and it is a string value that is
      // not empty
      if (isInput(property)) {
    return depth
  }
